...
export default function UserInfo() {
  ...
  /* 회원정보 수정 */
  const goUpdateInfo = async () => {
    if (nickname != "" && !isConfirmNickname) {
      alert("닉네임 중복확인 먼저 하세요.");
      return;
    }

    //null과 빈문자열 테이블에 넣을 때 잘 고려하기
    const res = await axios.post("/api/users", {
      url: "/updateUserInfo",
      no: no,
  ...
  /* 알림창닫기 */
  const goClose = () => {
    if (alertInfo.routerInfo) {
      router.push(alertInfo.routerInfo);
    }
    setAlertInfo({ open: false, content: "", routerInfo: "" });
  };

  return (
    <>
      {alertInfo.open ? (
        <Alert content={alertInfo.content} close={goClose} />
      ) : null}
          ...
          <div className="md:flex w-full">
            ...
            <div className="w-full md:w-1/2 py-10 px-5 md:px-10">
              <div className="text-center mb-10">
                <h1 className="font-bold text-3xl text-gray-900">Edit</h1>
              </div>
              <div>
